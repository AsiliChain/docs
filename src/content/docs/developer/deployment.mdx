...
Do this **before** rotating the multisig, not after.
:::
...
async function main() {
  ...
  // Step 8a: Set INDEPENDENT_AGGREGATOR
  // ⚠️ ROTATION WARNING: if this address ever changes post-launch,
  // existing independent farmer records are NOT auto-updated.
  // Call migrateFarmer() for each affected farmer before rotating.
  const INDEPENDENT_AGGREGATOR = process.env.INDEPENDENT_AGGREGATOR_ADDRESS!;
  if (!INDEPENDENT_AGGREGATOR)
    throw new Error('INDEPENDENT_AGGREGATOR_ADDRESS not set in .env');
  await farmerRegistry.setIndependentAggregator(INDEPENDENT_AGGREGATOR);
  console.log('INDEPENDENT_AGGREGATOR set to:', INDEPENDENT_AGGREGATOR);
  ...
  console.log(`INDEPENDENT_AGGREGATOR_ADDRESS=${INDEPENDENT_AGGREGATOR}`);
}
