...
```bash
curl https://api.asilichain.africa/eudr/dds/DDS-2026-004821 \
...
```bash
curl "https://api.asilichain.africa/eudr/dds?batch_id=BATCH-2026-004821" \
  -H "Authorization: Bearer {token}"
```
...
```bash
curl "https://api.asilichain.africa/eudr/dds?shipment_id=SHIP-2026-00042" \
  -H "Authorization: Bearer {token}"
```
