...
Triggers a merchant balance → UGX → MTN Mobile Money payout to a farmer via Fonbnk. Called automatically by the Alchemy webhook on EXPORTED event. Can also be called manually by a COOPERATIVE token for exceptional cases (e.g. net-only payment without a loan).
...
| Status | Meaning |
|--------|---------|
| `PROCESSING` | Fonbnk accepted, MTN MoMo pending |
| `COMPLETED` | Farmer received MTN MoMo notification |
| `FAILED` | Fonbnk returned error — retry queued |
...
| `FONBNK_UNAVAILABLE` | Fonbnk API unreachable after 3 attempts |
