...
Tokens are issued after KYB (Know Your Business) verification of the cooperative or organisation. Contact team@asilichain.africa.

```bash
# Exchange API key for Bearer token
curl -X POST https://api.asilichain.africa/auth/token \
...
```bash
curl -X POST https://api.asilichain.africa/auth/refresh \
...
|------------|---------|
| Production | `https://api.asilichain.africa` |
| Staging (Sepolia) | `https://staging-api.asilichain.africa` |
